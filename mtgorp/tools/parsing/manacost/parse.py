...
from mtgorp.models.persistent.attributes.manacosts import HybridCostAtom, ManaCost
from mtgorp.tools.parsing.exceptions import ParseException
from mtgorp.tools.parsing.manacost.gen.manacost_grammarLexer import (
    manacost_grammarLexer,
)
from mtgorp.tools.parsing.manacost.gen.manacost_grammarParser import (
    manacost_grammarParser,
)
from mtgorp.tools.parsing.manacost.visitor import (
    HybridBuilder,
    ManaCostBuilder,
    ManaCostVisitor,
)
...
class ManaCostParser(object):
    ...
    def _build(cls, parsed) -> t.Union[ManaCost, HybridCostAtom]:
        return (ManaCost if isinstance(parsed, ManaCostBuilder) else HybridCostAtom)(
            (cls._build(item) if isinstance(item, ManaCostBuilder) or isinstance(item, HybridBuilder) else item)
            for item in parsed
        )

    def parse(self, s: str) -> ManaCost:
        parser = manacost_grammarParser(CommonTokenStream(manacost_grammarLexer(InputStream(s))))
