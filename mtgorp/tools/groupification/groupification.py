...
class Groupifyer(ABC, t.Generic[T]):
	...
	def __init__(
		...
		self._name = name
		self._categories = list(categories) #type: t.List[Category]
...
class Group(t.Generic[T]):
	...
	def __init__(self, name: str, items: t.Iterable[T]):
		self._name = name
		self._items = Multiset(items) #type: Multiset[T]
...
NON_CREATURE_PERMANENTS_CATEGORY = Category(
	'Non-Creature Permanents',
	CriteriaBuilder().type_line.contains.no(typeline.LAND).all(),
)
...
STANDARD_PRINTING_GROUPIFYER = PrintingGroupifyer(
	(
		...
		NON_CREATURE_PERMANENTS_CATEGORY,
		LANDS_CATEGORY,
	),
)