...
from orp.database import OrpTable
    ...
    def get_cardboard_card_names(cls, raw_cardboard) -> t.Tuple[t.Sequence[str], t.Sequence[str]]:
        try:
            ...
            if raw_card_layout in (
                ...
                return (
                    tuple(raw_card["faceName"] for raw_card in sorted(raw_cardboard, key=lambda c: c["side"])),
                    (),
                )

            elif raw_card_layout in (Layout.TRANSFORM, Layout.MODAL):
                return tuple((name,) for name in raw_cardboard[0]["name"].split(" // "))
...
class ExpansionParser(ModelParser[E]):
    ...
    def __init__(
        ...
        self._default_booster_key_with_mythic = boosterkey.Parser.parse(
            [["rare", "mythic rare"]] + ["uncommon"] * 3 + ["common"] * 10 + ["land"]
        )
    ...
    def parse(
        ...
        release_date = (
            datetime.datetime.strptime(raw_expansion["releaseDate"], "%Y-%m-%d")
            if "releaseDate" in raw_expansion
            else None
        )
        ...
        expansion = self._target(
            ...
            booster_key=(
                ...
                if code in information and "booster_key" in information[code]
                else (
                    boosterkey.Parser.parse(raw_expansion["boosterV3"])
                    if "boosterV3" in raw_expansion
                    else (
                        self._default_booster_key_with_mythic
                        if release_date >= self._mythic_release_date
                        else self._default_booster_key
                    )
                )
            ),
            ...
            fragment_dividers=(tuple(information[code].get("fragment_dividers", ())) if code in information else ()),
        )
